 ...
 * 
 * 2016.08.23增加：
 * @params: imgAlt 轮播时图片下方是否显示注释说明，注释说明由img的alt获取。默认imgAlt为false，当其为true时才起作用
 */
;(function(global,factory){
	  if(typeof define === 'function' && define.amd){
		  define(["jquery"], factory);
		  //mdefine(["mjquery","css!utils.css","bootstrap","slimScroll/jquery.slimscroll.min"], factory);
	  }else{
		  if(typeof jQuery !== "undefined"){
			  global["util"] = factory(jQuery);
			}else{
				throw new Error("utils  require jQuery! ");
			}
	  }
  ...
  $.flexslider = function(el, options) {
    ...
    slider.setup = function(type) {
       //图片说明[2016.08.23]
		if(slider.vars.imgAlt==true){
			var img=$(slider.vars.selector, slider).find("img");
			img.each(function(){
				var altText=$(this).attr("alt");
				if(altText!=undefined){
					var $altDiv=$("<div class='flex-title' title='"+altText+"'></div>").text(altText);
					$altDiv.appendTo($(this).parents(slider.vars.selector, slider));
				}
			})
		}
      // SLIDE:
