/**
 * Created by lovercy on 23/11/2016.
 */
require.config(config);
require(["jquery","validate","base","animation","layer","jqueryForm","city.select","datepicker"],function($,validate,base,animation,layer,jqueryForm){
    var urlObj={
    ...
    function _submit(){
        ...
        var re=$("input[name='isLongTerm']:checked").val();
        if(re=="1"){
            var charterStartDate=$("#charterStartDate").val();
            var charterEndDate=$("#charterEndDate").val();
        ...
        var flag = true;
        $("#supplierForm [name]").each(function() {
            if(!checkForm($(this))){
                $(this).focus();
                flag = false;
                return false;
        ...
        if($("#wholesale_name").val()!=""&&$("#licenceWholesaleUrl").val()==""){
            $("#wholesale_name").focus();
            layer.msg("请上传附件",{icon:0});
            $("#submit").removeAttr("disabled");
            return false; 
        }
        /*if($("#fixedPoint_name").val()!=""&&$("#fixedPointUrl").val()==""){
            $("#fixedPoint_name").focus();
            layer.msg("请上传附件",{icon:0});
            $("#submit").removeAttr("disabled");
            return false; 
        }*/
        ...
        var params=base.serializeObject($("#supplierForm"));
        $.ajax({
            url:"/supplier/submit",
            data: {supplier:JSON.stringify(params)},
            type: 'POST',
            dataType: "json",
            success: function(data) {
                if(data.result==1){
                    animation.destory({
                        callback:function () {
                            layer.msg("数据提交成功",{icon:1},function(){
                                location.href=urlObj.homePage;
                            });
                        }
                    })
                }else{
    ...
    function _save() {
        ...
        if($("#logo_name").val()!=""&&$("#logoUrl").val()==""){
            $("#logo_name").focus();
            layer.msg("请上传公司Logo",{icon:0});
            $("#save").removeAttr("disabled");
            return false;
        }
        if($("#charter_name").val()!=""&&$("#charterUrl").val()==""){
            $("#charter_name").focus();
            layer.msg("请上传工商营业执照编号",{icon:0});
            $("#save").removeAttr("disabled");
            return false;
        }
        /*if($("#business_name").val()!=""&&$("#licenceBusinessUrl").val()==""){
            $("#business_name").focus();
            layer.msg("请上传食品流通许可证",{icon:0});
            $("#save").removeAttr("disabled");
            return false;
        }*/
        if($("#wholesale_name").val()!=""&&$("#licenceWholesaleUrl").val()==""){
            $("#wholesale_name").focus();
            layer.msg("请上传食盐批发许可证",{icon:0});
            $("#save").removeAttr("disabled");
            return false;
        }
        if($("#fixedPoint_name").val()!=""&&$("#fixedPointUrl").val()==""){
            $("#fixedPoint_name").focus();
            layer.msg("请上传食盐定点许可证",{icon:0});
            $("#save").removeAttr("disabled");
            return false;
        }
/*        animation.load({
        ...
        $.ajax({
            url:"/supplier/save",
            data: {supplierTemp:JSON.stringify(params)},
            type: 'POST',
            dataType: "json",
            success: function(data) {
                if(data.result==1){
                    layer.msg("数据保存成功",{icon:1},function(){
                        location.href=urlObj.homePage;
                    });
            ...
            error:function(){
                animation.destory({
                    callback:function(){
                        layer.msg("数据保存失败",{icon:2},function(){
                            //location.href=urlObj.homePage;
                        });
                    }
                });
    ...
    $(function() {
         ...
         *失去交点验证
         */
        $("#supplierForm [name]").blur(function(){
            checkForm($(this));
        });
        ...
        $("input[name='isLongTerm']").on("change", function () {
            var selectVal=$("input[name='isLongTerm']:checked").val();
            if(selectVal=="0"){
                $("#isLongTermLi").css("visibility","hidden");
                $("#isLongTermLi").find("input").removeClass("required");
            }else{
                $("#isLongTermLi").css("visibility","visible");
                $("#isLongTermLi").find("input").each(function(){
                    $(this).val("");
                    $(this).attr("value","");
...
*/
        $(".js-submit").on("click", function () {
            var conf = layer.confirm('请确认是否提交新的信息',{
                btn: ['确定','取消']
            }, function(){
                _submit();
                layer.close(conf);
        ...
        $("#province,#city").citylist({
            ...
            metaTag: 'name',
            idVal: true,
            selected:[$("#province").attr("provinceId"),$("#city").attr("cityId")]
        });
        ...
        /**
         * 根据选择的证件类型不同,为证件号添加不同的校验规则
         */
        $("#credentialType").on("change",function(){
            var type=$("#credentialType").val();
            $("#credentialCode").removeClass();
            if(type && type=='1'){
                $("#credentialCode").addClass("ds-inp-control required isIDCard");
            }else if(type && type=='2'){
                $("#credentialCode").addClass("ds-inp-control required isPassPort");
            }else if(type && type=='3'){
                $("#credentialCode").addClass("ds-inp-control required isHKMacao");
            }else{
                $("#credentialCode").addClass("ds-inp-control required");
            }
        })
        animation.destory();
    })
