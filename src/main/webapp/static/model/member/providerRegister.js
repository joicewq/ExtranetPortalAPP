...
require(["jquery","validate","local","animation","layer","city.select","datepicker"],function($,validate,local,animation,layer){
	var urlObj={
    ...
    function _submit(){
        var flag = true;
        $("#providerForm [name]").each(function() {
            if(!checkForm($(this))){
                $(this).focus();
                flag = false;
                return false;
        ...
        if(flag){
            ...
            var params=base.serializeObject($("#providerForm"));
            $.ajax({
                url:"/provider/submit",
                data: {provider:JSON.stringify(params)},
                type: 'POST',
                dataType: "json",
                        ...
                        animation.destory({
                            callback:function () {
                                layer.confirm("数据提交成功",{
                                	...
                                	location.href=urlObj.loginPage;
                                },function(){
    ...
    function _save() {
        ...
        $.ajax({
            url:"/provider/save",
            data: {providerTemp:JSON.stringify(params)},
            type: 'POST',
            dataType: "json",
            success: function(data) {
