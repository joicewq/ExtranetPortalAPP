...
<script type="text/javascript" src="${ctx}/static/model/prodlib/index.js"></script>
		...
		<![CDATA[
			...
			{#template ROW}
				<tr id="{$T.id}">
					...
					<td>{$T.stdNum}</td>
					<td>
						{#if $T.listedDate}{new Date($T.listedDate).Format('yyyy-MM-dd')}
						{#else }--
						{#/if}
					</td>
					<td>{$T.sta}</td>
					<td><a style="cursor:pointer;" onclick="show('{$T.id}')">详情</a></td>
				</tr>
			{#/template ROW}
		]]>
	</textarea>
